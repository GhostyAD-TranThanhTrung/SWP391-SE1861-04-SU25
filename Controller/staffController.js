  ...
  static async getAllStaff(req, res) {
    try {
      ...
      const staffUsers = await userRepository.find({
        where: [{ role: "admin" }, { role: "consultant" }],
        ...
        staffUsers.map(async (user) => {
          ...
          if (profile && profile.bio_json) {
            try {
              ...
              console.warn(
                `Failed to parse bio_json for user ${user.user_id}:`,
                error
              );
            }
          }
   ...
   */ static async createStaff(req, res) {
    ...
    try {
      ...
      if (name || bio_json || date_of_birth || job) {
        ...
        if (bio_json) {
          bioJsonString =
            typeof bio_json === "string" ? bio_json : JSON.stringify(bio_json);
        }
    ...
    try {
      ...
      if (hasProfileData) {
        ...
        if (bio_json !== undefined) {
          try {
            bioJsonString =
              typeof bio_json === "string"
                ? bio_json
                : JSON.stringify(bio_json);
          } catch (error) {
            return res.status(400).json({
              success: false,
        ...
        if (updatedProfile) {
          ...
          if (bioJsonString !== undefined)
            updatedProfile.bio_json = bioJsonString;
          if (date_of_birth !== undefined)
            updatedProfile.date_of_birth = date_of_birth
